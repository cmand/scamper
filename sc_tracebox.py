...
class WartsTraceBoxPkt(WartsBaseObject):
    def __init__(self, data, refs, verbose=False):
        ...
        self.flags["data"] = self.read_tracebox_pkt(
            data[self.flag_bytes : self.flag_bytes + datalen]
        )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if flags == False:
            break
        print(
            "tracebox from %s to %s (result: %d)"
            % (flags["srcaddr"], flags["dstaddr"], flags["result"])
        )
        last_tx = None
        last_tx_ts = 0
        i = 0
        for pkt in pkts:
            ...
            if pkt["dir"] == 1:  # TX
                ...
                if last_tx:
                    ...
                    print(
                        " %d: %s RTT:%1.3f: %s"
                        % (i, pkt["data"]["hop"], rtt, " ".join(list(diff.keys())))
                    )
                last_tx = None
