    ...
    def stats(self, verbose=False, count=0):
        while True:
            ...
            if verbose and (self.cnt % 1000 == 0):
                print(
                    ">> %s (traces:%d/dests:%d/ints:%d/edges:%d)"
                    % (
                        self.wartsfile,
                        self.cnt,
                        len(self.dests),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        print(
            "\tInterfaces in both %s and %s: %d"
            % (w1.wartsfile, w2.wartsfile, len(w1.ints & w2.ints))
        )
        print(
            "\tInterfaces in %s not in %s: %d"
            % (w1.wartsfile, w2.wartsfile, len(w1.ints - w2.ints))
        )
        print(
            "\tInterfaces in %s not in %s: %d"
            % (w2.wartsfile, w1.wartsfile, len(w2.ints - w1.ints))
        )
        print(
            "\tEdges in both %s and %s: %d"
            % (w1.wartsfile, w2.wartsfile, len(w1.edges & w2.edges))
        )
        print(
            "\tEdges in %s not in %s: %d"
            % (w1.wartsfile, w2.wartsfile, len(w1.edges - w2.edges))
        )
        print(
            "\tEdges in %s not in %s: %d"
            % (w2.wartsfile, w1.wartsfile, len(w2.edges - w1.edges))
        )
