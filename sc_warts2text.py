...
def print_ping(flags, responses):
    rtts = []
    print(
        "ping %s to %s: %d byte packets"
        % (flags["srcaddr"], flags["dstaddr"], flags["size"])
    )
    rcount = set()
    for response in responses:
        ...
        rtts.append(rtt)
        print(
            "%d bytes from %s, seq=%d ttl=%d time=%3.3f ms"
            % (
                response["replysize"],
                response["addr"],
                response["probeid"],
    ...
    if len(rcount) > 0:
        print(
            "round-trip min/avg/max/stddev = %2.3f/%2.3f/%2.3f/%2.3f ms" % basic(rtts)
        )
    ...
    while True:
        try:
            ...
            if typ == None:
                ...
                if target and target != flags["dstaddr"]:
                    continue
